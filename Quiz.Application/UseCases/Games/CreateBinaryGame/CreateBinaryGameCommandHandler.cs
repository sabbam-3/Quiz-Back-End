...
internal sealed class CreateBinaryGameCommandHandler(
    IGameRepository gameRepository,
    IUserRepository userRepository,
    IQuoteRepository quoteRepository,
    IUserContext context,
    IUnitOfWork unitOfWork) : ICommandHandler<CreateBinaryGameCommand, Guid>
{
    public async Task<Result<Guid>> Handle(CreateBinaryGameCommand command, CancellationToken cancellationToken)
    {
        ...
        IReadOnlyCollection<string> allAuthorNames = await quoteRepository.GetAllDistinctAuthorNamesAsync(cancellationToken: cancellationToken);

        Result<Game> game = Game.CreateBinary(context.UserId, randomQuotes, allAuthorNames);
